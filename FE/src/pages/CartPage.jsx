...
const ADDRESS_HISTORY_KEY = 'fitflow_checkout_address_history'
      ...
      <div className="mx-auto flex min-h-[calc(100vh-140px)] max-w-3xl items-center justify-center px-4 py-12">
        <div className="w-full rounded-[28px] border border-white/70 bg-white/90 p-10 text-center shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur">
          ...
          <div className="mt-8 flex justify-center gap-3">
            ...
            <Link to="/buy?purpose=rent&openBooking=1" className="rounded-full border border-slate-200 px-6 py-3 font-semibold text-slate-700">Đặt lịch thuê vợt</Link>
          </div>
        </div>
      </div>
    </div>
...
export default function CartPage() {
  ...
  const [rentalPaymentMethod, setRentalPaymentMethod] = useState('PayOS')
  ...
  const rentalTotalItemValue = rentalItemsWithTotal.reduce((sum, item) => sum + (Number(item.baseSalePrice || 0) * (item.quantity || 1)), 0)
  const rentalDepositAmount = rentalPaymentMethod === 'Cash' 
    ? Math.min(100000, rentalSubtotalAfterVoucher) 
    : rentalTotalItemValue * 0.5
  const rentalRemainingAmount = Math.max(rentalTotalItemValue - rentalDepositAmount, 0)
  ...
  const handleCheckout = async ({ session = guestVerificationSession } = {}) => {
          ...
          <div>
            ...
            <div className="mt-2 flex items-center gap-3">
              ...
              <div>
                ...
                <h1 className="text-lg font-semibold text-slate-950 sm:text-xl">Hoàn tất đơn hàng FITFLOW</h1>
              </div>
            </div>
          </div>
              ...
              {rentalItems.length > 0 ? (
                <div className="rounded-3xl border border-sky-100 bg-sky-50/40 p-5">
                  <div className="space-y-3">
                    ...
                    <div className="border-t border-slate-200 pt-3">
                      <SummaryRow 
                        label={rentalPaymentMethod === 'Cash' ? 'Phí giữ vợt' : 'Đặt cọc (50%)'} 
                        value={formatCurrency(rentalDepositAmount)} 
                        emphasized 
                      />
                    </div>
                  </div>
                  ...
                  {['PayOS'].includes(rentalPaymentMethod) && (
      ...
      <GuestVerificationModal
        ...
        onSuccess={(payload) => {
          ...
          if (pendingGuestCheckout === 'combined') {
            setPendingGuestCheckout(null)
            handleCheckout({ session: nextSession })
          }
        }}
      />
    </div>
